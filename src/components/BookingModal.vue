...
const submitBooking = async () =>
{
    ...
    try
    {
        ...
        // Envíado real al webhook de n8n
        const response = await fetch(webhookBooking, {
            method: 'POST',
...
<template>
    <Teleport to="body">
        <div v-if="isOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <!-- Modal Content -->
            <div
                class="glass-card w-full max-w-2xl rounded-2xl relative z-10 overflow-hidden shadow-2xl animate-fade-in-up border border-white/10 text-white min-h-[500px] flex flex-col bg-[#111827]">
                <!-- Header -->
                <div class="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
                    ...
                    <button @click="closeModal"
                        class="text-gray-400 hover:text-white hover:bg-white/10 rounded-full p-2 transition">
                ...
                <div class="flex-1 p-6 overflow-y-auto relative">
                    ...
                    <div v-if="step === 1">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <button v-for="day in availableSlots" :key="day.date" @click="selectDate(day)"
                                ...
                                <span class="text-xs text-gray-400 uppercase group-hover:text-primary-light">{{
                                    day.display.split(' ')[0] }}</span>
                    ...
                    <div v-else-if="step === 2">
                        ...
                        <div class="grid grid-cols-3 sm:grid-cols-5 gap-3">
                            <button v-for="slot in selectedDate.slots" :key="slot" @click="selectTime(slot)"
                        ...
                        <form @submit.prevent="submitBooking" class="space-y-4">
                            <div>
                                ...
                                <input v-model="formData.name" type="text" required
                                    class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-primary-light focus:ring-1 focus:ring-primary-light transition text-white placeholder-gray-500"
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-400 mb-1">¿Qué proceso te gustaría
                                    automatizar?</label>
                                <textarea v-model="formData.notes" rows="3"
                                    class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-primary-light focus:ring-1 focus:ring-primary-light transition text-white placeholder-gray-500"></textarea>
                            </div>

                            <button type="submit"
                                class="w-full bg-primary hover:bg-primary-light text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/20 transition transform hover:-translate-y-0.5">
                                Confirmar Reunión
                            </button>
                        </form>
                    </div>
                    ...
                    <div v-else-if="step === 4"
                        ...
                        <p class="text-gray-400 mb-8 max-w-xs mx-auto">Te hemos enviado un correo de confirmación con el
                            link de la videollamada.</p>
                        <button @click="closeModal"
                            class="bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-lg transition">Cerrar</button>
                    </div>
                    ...
                    <div v-else-if="step === 5"
                        ...
                        <p class="text-gray-400 mb-8 max-w-xs mx-auto">No pudimos procesar tu solicitud. Por favor
                            intenta nuevamente.</p>
                        <button @click="step = 3" class="text-primary-light hover:text-white underline">Volver a
                            intentar</button>
                    </div>
